...
async def online_estimates_put(file: UploadFile, label: str = Form(...), full_name: str = Form(...), vin: str = Form(...)  ):
    ...
    try:
        key = f"online-estimates/{safe_name}_{label}_{vin}{mimetypes.guess_extension(file.content_type) or ''}"
